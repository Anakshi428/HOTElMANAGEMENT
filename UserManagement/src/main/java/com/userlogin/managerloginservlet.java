...
protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
{
    ...
    try
    {
        ...
        if(userValidate.equals("User_manager"))
        {
            System.out.println("User's Home");
 
            HttpSession session = request.getSession(); //Creating a session
            session.setAttribute("Userr", managerID); //setting session attribute
            request.setAttribute("userName", managerID);
 
            request.getRequestDispatcher("SearchUserManager.jsp").forward(request, response);
        }
